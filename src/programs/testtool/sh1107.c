 ...
 * sh1107_i2c.c
 *
 * SH1107 OLED driver over I2C, built on top of the bit-banged I2C
 * master in i2c_bitbang_68230.c (SDA -> PC0, SCL -> PC1 on the
 * 68230's Port C). Compile and link this together with that file.
 *
 ...
 * 0x3D (SA0 tied high) -- change SH1107_I2C_ADDR if yours doesn't
 * respond.
 *
...
{
    PGCR = 0x00;                    /* Port B: Mode 0, bit I/O */
    PBDDR |= RES_MASK;              /* RES pin as output */
    PBDR |= RES_MASK;               /* idle high */
    reset_delay();
    PBDR &= (uint8_t)~RES_MASK;     /* pulse low */
    reset_delay();
    PBDR |= RES_MASK;               /* release */
    reset_delay();
}
...
/*-----------------------------------------------------------------------
 * DEMO / test program (build with -DSH1107_DEMO)
 *-----------------------------------------------------------------------*/
#ifdef SH1107_DEMO
#include <stdio.h>
